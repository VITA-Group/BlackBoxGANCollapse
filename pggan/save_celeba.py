...
class ImageExporter:
    ...
    def add_image(self, img, idx):
        ...
        self.cur_images += 1
        PIL.Image.fromarray(quant, 'RGB').save(os.path.join('celebahq', '%06d.png' % idx))
...
def create_celebahq(tfrecord_dir, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    with ImageExporter(tfrecord_dir, indices.size) as ImExp:
        order = ImExp.choose_shuffled_order()
        with ThreadPool(num_threads) as pool:
            for img,idx in pool.process_items_concurrently(indices[order].tolist(), process_func=process_func,
