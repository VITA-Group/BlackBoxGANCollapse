...
def training_schedule(
    ...
    tick_kimg_dict          = {4: 160, 8:140, 16:120, 32:100, 64:80, 128:60, 256:40, 512:30, 1024:1}): # Resolution-specific overrides.
...
def training_loop(
    ...
    import pickle
    args = EasyDict(tfrecord_dir='1m_0.22_clustered_images_tfrecords', resolution=1024)
    collapsed_set = dataset.load_dataset(data_dir=config.data_dir, verbose=True, **args)
    resume_kimg = 12000
    total_kimg = 15000
    def load_Gs(url):
    ...
    with tf.device('/gpu:0'):
        G, D, Gs = load_Gs('https://drive.google.com/uc?id=1MGqJl28pN4t7SAtSrPdSRJSQJqahkzUf')
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            collapsed, _ = collapsed_set.get_minibatch_tf()
            reals = process_reals(reals, lod_in, mirror_augment, training_set.dynamic_range, drange_net)
            collapsed = process_reals(collapsed, lod_in, mirror_augment, collapsed_set.dynamic_range, drange_net)
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set, minibatch_size=minibatch_split, collapsed=collapsed, **G_loss_args)
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals, labels=labels, collapsed=collapsed, **D_loss_args)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        collapsed_set.configure(sched.minibatch // submit_config.num_gpus, sched.lod)
