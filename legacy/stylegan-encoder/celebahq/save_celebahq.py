...
def save_images(training_set):
    ...
    while True:
        ...
        reals = PIL.Image.fromarray(reals, 'RGB')
        reals = reals.crop((112, 112, 912, 912))
        reals = reals.resize((112, 112), PIL.Image.ANTIALIAS)
        reals.save(os.path.join('celebahq-raw', '%06d.png' % count), 'PNG')
        count += 1
...
def main(
    ...
    training_set = dataset.load_dataset(data_dir='datasets/', verbose=True, tfrecord_dir='celebahq')
