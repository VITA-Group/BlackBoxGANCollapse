...
def generate_from_npy(Gs):
    ...
    print('start loading')
    latents_mat = np.stack([np.load(file) for file in latents_files[:600]], axis=0)
    print('end loading')
    test_latents = np.stack([np.load(file) for file in latents_files[600:]], axis=0)
    #latents_mat = np.random.rand(29,512)
    #print(latents_mat.shape)
    count = 0
    for i in range(test_latents.shape[0]):
        print(i)
        #latent = draw_from_convex_hull(latents_mat)
        latent = test_latents[i]
        #if in_hull(latent[np.newaxis,:], latents_mat):
        if in_hull(latents_mat, np.squeeze(latent)):
            print(i)
