...
def generate_from_npy(Gs):
    ...
    save_dir = 'monte_carlo_sampling_10m_celebahq/neighbors/0.2/30576/collision/lerp'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    path = 'monte_carlo_sampling_10m_celebahq/neighbors/0.2/30576/collision/neighbors_latents'
    ...
    for i in range(50):
        ...
        dst = os.path.join(save_dir, '{}_{}.png'.format(npys[m].split('/')[-1][:-4].split('_')[0], npys[n].split('/')[-1][:-4].split('_')[0]))
        print(dst)
        image.save(dst, 'PNG')
