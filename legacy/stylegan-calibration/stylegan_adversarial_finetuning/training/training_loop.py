...
def training_loop(
    ...
    with tf.device('/gpu:0'):
        G, D, Gs = load_Gs('/mnt/ilcompf5d1/user/zwu/stylegan_adversarial_finetuning/results/00012-sgan-celebahq-4gpu_none_128/network-snapshot-012000.pkl')
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            collapsed = pickle.load(open('/mnt/ilcompf5d1/user/zwu/stylegan_adversarial_finetuning/collapsed.pkl', 'rb'))[np.newaxis, :]
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set,
                                                       minibatch_size=minibatch_split, **G_loss_args, collapsed=collapsed)
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set,
                                                       minibatch_size=minibatch_split, reals=reals, labels=labels, **D_loss_args)
    ...
    saver.restore(sess, '/mnt/ilcompf5d1/user/zwu/stylegan_adversarial_finetuning/InsightFace/pretrained/config_ms1m_100_334k/best-m-334000')
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for _mb_repeat in range(minibatch_repeats):
            for _D_repeat in range(D_repeats):
                tflib.run([D_train_op, Gs_update_op],
