...
def G_logistic_nonsaturating(G, D, opt, latents, reals, training_set, minibatch_size): # pylint: disable=unused-argument
    labels = tf.zeros([minibatch_size, 512], training_set.label_dtype)
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_scores_out = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    loss = tf.nn.softplus(-fake_scores_out)  # -log(logistic(fake_scores_out))
    loss += tf.losses.mean_squared_error(fake_images_out, reals)
    return loss
...
def D_logistic_simplegp(G, D, opt, training_set, latents, minibatch_size, reals, r1_gamma=10.0, r2_gamma=0.0): # pylint: disable=unused-argument
    #latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = tf.zeros([minibatch_size, 512], training_set.label_dtype)
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    real_scores_out = fp32(D.get_output_for(reals, labels, is_training=True))
