...
def create_from_images_latents(tfrecord_dir, image_dir, latent_dir, shuffle):
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '**', '*.png'), recursive=True))
    ...
    print('Loading latents from "%s"' % latent_dir)
    latent_filenames = sorted(glob.glob(os.path.join(latent_dir, '**', '*.npy'), recursive=True))
    if len(latent_filenames) == 0:
        error('No input latents found')
    latent_lst = []
    for file in latent_filenames:
        latent = np.load(file)
        latent_lst.append(latent)
    latents = np.asarray(latent_lst)
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_from_images_latents', 'Create dataset from a directory full of images and latents for knowledge distillation.',
                                            'create_from_images datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     'latent_dir',       help='Directory containing the latents')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
