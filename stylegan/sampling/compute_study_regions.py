...
if __name__ == "__main__":
    ...
    print(Robs_indices)
    Rref_anchors_embds_dct, Robs_anchors_embds_dct = {}, {}
    Rref_anchors_latents_dct, Robs_anchors_latents_dct = {}, {}
    ...
    for ind in Robs_indices:
        ...
        with open(pkl_dir, 'rb') as handle:
            ...
            embds = pkl[ind % M]
            Robs_anchors_embds_dct[ind] = embds / np.linalg.norm(embds)
        ...
        Robs_anchors_latents_dct[ind] = handle[ind % M]

    with open(os.path.join(path, 'neighbors', 'Robs_anchors_embds_dct_{}.pkl'.format(theta)), 'wb') as handle:
        pickle.dump(Robs_anchors_embds_dct, handle)
    with open(os.path.join(path, 'neighbors', 'Robs_anchors_latents_dct_{}.pkl'.format(theta)), 'wb') as handle:
        pickle.dump(Robs_anchors_latents_dct, handle)
    ...
    for ind in Rref_indices:
        ...
        with open(pkl_dir, 'rb') as handle:
            pkl = pickle.load(handle)
            embds = pkl[ind % M]
            Rref_anchors_embds_dct[ind] = embds / np.linalg.norm(embds)

        npy_dir = os.path.join(os.path.join(path, 'latents'), '{}_{}'.format((ind // D) * D, (ind // D + 1) * D),
                               '{}_{}.npy'.format((ind // M) * M, (ind // M + 1) * M))
        handle = np.load(npy_dir)
        Rref_anchors_latents_dct[ind] = handle[ind % M]

    with open(os.path.join(path, 'neighbors', 'Rref_anchors_embds_dct_{}.pkl'.format(theta)), 'wb') as handle:
        pickle.dump(Rref_anchors_embds_dct, handle)
    with open(os.path.join(path, 'neighbors', 'Rref_anchors_latents_dct_{}.pkl'.format(theta)), 'wb') as handle:
        pickle.dump(Rref_anchors_latents_dct, handle)
