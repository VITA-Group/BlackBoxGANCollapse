...
class ImageExporter:
    def __init__(self, save_dir, expected_images, print_progress=True, progress_interval=10):
        self.save_dir = save_dir
        self.tfr_prefix = os.path.join(self.save_dir, os.path.basename(self.save_dir))
        ...
        if self.print_progress:
            print('Creating dataset "%s"' % save_dir)
    ...
    def add_image(self, img, idx):
        ...
        quant = quant.transpose(1, 2, 0)  # CHW -> HWC
        img = PIL.Image.fromarray(quant, 'RGB')
        # img = img.crop((112, 112, 912, 912))
        # img = img.resize((112, 112), PIL.Image.ANTIALIAS)
        img.save(os.path.join(self.save_dir, '%06d.png' % idx))
...
def create_celebahq(save_dir, celeba_dir, delta_dir, start, end, num_threads=4, num_tasks=100):
    print('Loading CelebA from "%s"' % celeba_dir)
    expected_images = 202599
    found_imgs = len(glob.glob(os.path.join(celeba_dir, 'img_celeba', '*.jpg')))
    if found_imgs != expected_images:
        error('Expected to find %d images but actually find %d images' % (expected_images, found_imgs))
    ...
    indices = indices.tolist()[start:end]
    with ImageExporter(save_dir, len(indices)) as ImExp:
        #order = ImExp.choose_shuffled_order()
        with ThreadPool(num_threads) as pool:
            for img,idx in pool.process_items_concurrently(indices, process_func=process_func,
                                                       max_items_in_flight=num_tasks):
                ImExp.add_image(img, idx)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Saving celeba')
    parser.add_argument('--start', help='Starting index for creating CelebaHQ', type=int)
    parser.add_argument('--end', help='Ending index for creating CelebaHQ', type=int)
    args, other_args = parser.parse_known_args()
    save_dir = '../datasets/celebahq-dataset/CelebAHQ'
    celeba_dir = '../datasets/celebahq-dataset/CelebA'
    delta_dir = '../datasets/celebahq-dataset/celeba-hq-deltas'
    create_celebahq(save_dir, celeba_dir, delta_dir, args.start, args.end, num_threads=32, num_tasks=100)