...
#data_dir = '/mnt/ilcompf5d1/user/zwu/progressive_growing_of_gans/datasets'
data_dir = '/ssd1/wuzhenyu_sjtu/ffhq-dataset/tfrecords'
result_dir = 'results'
...
invert      = EasyDict(func='invert.invert_progressive_gan')
save_celebahq = EasyDict(func='save_celebahq.save_celebahq')
G           = EasyDict(func='networks.G_paper')             # Options for generator network.
D           = EasyDict(func='networks.D_paper')             # Options for discriminator network.
G_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for generator optimizer.
D_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for discriminator optimizer.
invert_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for discriminator optimizer.
G_loss      = EasyDict(func='loss.G_wgan_acgan')            # Options for generator loss.
D_loss      = EasyDict(func='loss.D_wgangp_acgan')          # Options for discriminator loss.
invert_loss = EasyDict(func='loss.G_invert')
sched       = EasyDict()                                    # Options for train.TrainingSchedule.
grid        = EasyDict(size='1080p', layout='random')       # Options for train.setup_snapshot_image_grid().

# Dataset (choose one).
#desc += '-celebahq';            dataset = EasyDict(tfrecord_dir='celebahq', resolution=1024); train.mirror_augment = True
desc += '-ffhq';                  dataset = EasyDict(tfrecord_dir='ffhq', resolution=1024); train.mirror_augment = True
...
#desc += '-preset-v2-1gpu'; num_gpus = 1; train.total_kimg = 12000
...
desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4, 1024: 4}; sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
