...
class TFRecordExporter:
    ...
    def add_latents(self, latents):
        if self.print_progress:
            print('%-40s\r' % 'Saving latents...', end='', flush=True)
        assert latents.shape[0] == self.cur_images
        with open(self.tfr_prefix + '-rxx.latents', 'wb') as f:
            np.save(f, latents.astype(np.float32))
...
def create_celebahq(tfrecord_dir, celeba_dir, delta_dir, num_threads=4, num_tasks=100, attribute=None):
    print('Loading CelebA from "%s"' % celeba_dir)
    expected_images = 202599
    ...
    if attribute is None:
        file = open(os.path.join(delta_dir, 'image_list.txt'), 'rt')
    elif attribute == 'Age':
        file = open(os.path.join(delta_dir, 'image_list_with_fairness_attributes_age_balanced.txt'), 'rt')
    elif attribute == 'Gender':
        file = open(os.path.join(delta_dir, 'image_list_with_fairness_attributes_gender_balanced.txt'), 'rt')
    elif attribute == 'Race':
        file = open(os.path.join(delta_dir, 'image_list_with_fairness_attributes_race_balanced.txt'), 'rt')
    elif attribute == 'Identity':
        file = open(os.path.join(delta_dir, 'image_list_with_fairness_attributes_identity_balanced.txt'), 'rt')
    else:
        raise ValueError('No given specific fairness attribute')
    ...
    indices = np.arange(len(fields['idx']))
    ...
    def process_func(idx):
        ...
        with zipfile.ZipFile(os.path.join(delta_dir, 'deltas%05d.zip' % (fields['idx'][idx] - fields['idx'][idx] % 1000)), 'r') as zip:
            delta_bytes = zip.read('delta%05d.dat' % fields['idx'][idx])
...
def execute_cmdline(argv):
    ...
    p.add_argument(     '--attribute',      help='Balanced fairness attribute', type=str, default=None)
